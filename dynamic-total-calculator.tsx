...
const TotalCalculator = () => {
  ...
  useEffect(() => {
    // 初始化30行，數量預設為1
    setRows(Array.from({ length: 30 }, (_, i) => ({
      id: i + 1,
      price: 0,
      quantity: 1  // 預設數量為1
    })));
  ...
  const handleChange = (id, field, value) => {
    setRows(prev => prev.map(row => {
      if (row.id === id) {
        // 確保數量至少為1
        const newValue = field === 'quantity' ? Math.max(1, parseInt(value) || 1) : parseFloat(value) || 0;
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-bold mb-6 text-center text-blue-600">總額: {total.toFixed(2)}</h1>
      <div className="space-y-4">
        {rows.map(row => (
          <div key={row.id} className="flex items-stretch bg-white rounded-lg shadow-md p-4">
